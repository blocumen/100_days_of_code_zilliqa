...
class Home extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      if (Zilliqa) {
        console.log("xonnected succesfully");
        console.log("default address is - " + Zilliqa.wallet.defaultAccount.bech32);
      }

      const accounts = Zilliqa.wallet.defaultAccount.bech32;
      const web3 = Zilliqa.wallet.defaultAccount.bech32;
  ...
  CreateNFT() {
    ...
    const msgVersion = 1;
    var VERSION = Zilliqa.utils.bytes.pack(ChainId, msgVersion);
    var myGasPrice = Zilliqa.utils.units.toQa('2000', Zilliqa.utils.units.Units.Li);
  ...
  render() {
    ...
    return (
      ...
      <div>
          <h4>My account zilPay address - {this.state.accounts}</h4>
      </div>
        ...
        <a href={`https://gateway.ipfs.io/ipfs/${this.state.ipfsHash}`} > View on IPFS</a>

        <div>
            <h4>Deploy the nftname </h4>
        </div>
        <h2> Mint an Nft </h2>
        <form id="IDCreateNFT" class="scep-form" onSubmit={this.CreateNFT}>
          <label for="tokenname">Nft name</label>
          <input type="text" id="nftname" placeholder="your NFT name" />
          <label for="tokensymbol">&nbsp; Nft symbol</label>
          <input type="text" id="nftsymbol" placeholder="your NFT symbol" />
          <label for="tokenname">&nbsp; Nft location</label>
          <input type="text" id="ipfshash" value={this.state.ipfsHash} />
          <label for="category">&nbsp; Choose your Option</label>

          <select value={this.state.timeperiod} onChange={this.handleChangeTimePeriod}>
            <option value="week">A week</option>
            <option value="month">Bi-annual</option>
